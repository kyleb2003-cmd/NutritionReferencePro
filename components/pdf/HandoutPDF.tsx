...
const styles = StyleSheet.create({
  ...
  titleClinic: { fontSize: 14, fontWeight: 700, color: '#111827' },
  titleApp: { fontSize: 10, color: '#6B7280' },
  titleCondition: { fontSize: 16, fontWeight: 700, color: '#111827', marginTop: 2 },
  ...
  footerLeft: { maxWidth: '75%', fontSize: 10, color: '#111827' },
  footerRight: { fontSize: 10, color: '#111827', textAlign: 'right' },
  debugBorder: { borderWidth: 1, borderColor: '#EF4444' },
  debugText: { fontSize: 10, color: '#6B7280', marginBottom: 6 },
})
...
export default function HandoutPDF(props: {
  ...
  const { clinicName, footerText, logoDataUrl, conditionName, patientName, printedOn, sections, debug } = props
  const normalized = normalizeSections(sections)

  return (
    <Document>
      <Page size="LETTER" style={styles.page} wrap>
        <View fixed style={debug ? [styles.header, styles.debugBorder] : styles.header}>
          {logoDataUrl ? (
        ...
        <View fixed style={debug ? [styles.footer, styles.debugBorder] : styles.footer}>
        ...
        <View style={debug ? [styles.content, styles.debugBorder] : styles.content}>
          {debug ? <Text style={styles.debugText}>Sections to render: {normalized.length}</Text> : null}
          {normalized.map((section, index) => (
            <View key={`${section.label}-${index}`} break={index > 0} style={styles.sectionBlock}>
              <Text style={styles.sectionTitle}>{section.label}</Text>
              {renderMarkdown(section.blocks)}
            </View>
          ))}
        </View>
      </Page>
